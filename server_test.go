...
func dumpMetrics(t *testing.T, cfg *ServerConfig, args string) string {
	...
	url := fmt.Sprintf("http://%s/%s%s", cfg.Http.Listen, cfg.Http.TelemetryPath, args)
...
func testMetrics(t *testing.T, cfg *ServerConfig, args string) {
	...
	hasLogProxyMetrics := strings.Index(buf, "logproxy_received_bytes_total") != -1
...
func writeJson(t *testing.T, pserver *ProtocolServer) {

	tcpConn, err := net.DialTCP("tcp", nil,
		&net.TCPAddr{IP: []byte{127, 0, 0, 1}, Port: int(testJsonPort)})
	if err != nil {
		t.FailNow()
	}
	tcpConn.SetNoDelay(true)
	...
	tcpConn.Write(data)
	tcpConn.Close()
	fmt.Printf("Wrote %d bytes, 3 json messages\n", len(data))
...
func TestProtocols(t *testing.T) {
	...
	cfg := &ServerConfig{
		...
		Protocols: []ProtocolConfig{
			{Type: "json", Enable: true, Host: "127.0.0.1", Port: int(testJsonPort)},
	...
	t.Run("proto-json", func(t *testing.T) { writeJson(t, server.pservers[0]) })
	t.Run("proto-log", func(t *testing.T) { writeLines(t, server.pservers[1]) })
	t.Run("proto-fluent", func(t *testing.T) { testFluent(t, server.pservers[2]) })
